...
void CMCPforNTApp::OnViewHardwaresetup() 
{
	...
	pdlg->m_ferddichsubadd	= FerddichSubadd;
	pdlg->m_flukegpib		= FlukeGpib;
	pdlg->m_tempgpib		= TempGpib;
	pdlg->m_fieldgpib		= FieldGpib;
	pdlg->m_gelbeboxslot	= GelbeboxSlot;
	pdlg->m_gelbebox2slot	= Gelbebox2Slot;
	pdlg->m_gpibslot		= GpibSlot;
	pdlg->m_hp1gpib			= Hp1Gpib;
	pdlg->m_hp2gpib			= Hp2Gpib;
	pdlg->m_listslot		= ListSlot;
	pdlg->m_premaslot		= PremaSlot;
	pdlg->m_presetslot		= PresetSlot;
	pdlg->m_protonslot		= ProtonSlot;
	pdlg->m_protonsubadd	= ProtonSubadd;
	pdlg->m_rs1gpib			= Rs1Gpib;
	pdlg->m_rs2gpib			= Rs2Gpib;
	pdlg->m_scalerslot		= ScalerSlot;
	pdlg->m_triggersubadd	= TriggerSubadd;
	pdlg->m_hpvoltslot		= HpSlot;
	pdlg->m_hvvoltslot		= HvSlot;
	pdlg->m_fvoltslot		= FSlot;
	pdlg->m_premadelay		= PremaDelay;
	pdlg->m_prema_post_delay= PremaPostDelay;
	pdlg->m_flukedelay		= FlukeDelay;
	pdlg->m_massdelay		= MassDelay;
	pdlg->m_gpibdelay		= GPIBDelay;
	pdlg->m_dip_ht			= DipHT;
	pdlg->m_dip_pc			= DipPC;
	pdlg->m_dip_ht_value	= DipHTValue;
	pdlg->m_dip_pc_value	= DipPCValue;
	pdlg->m_SICLReaderSubAdd= SICLReaderBit;
	pdlg->m_SICLStepSubAdd	= SICLStepBit;
