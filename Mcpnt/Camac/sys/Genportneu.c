...

{
	...
	extension->ActualStringPosPtr		= 0;
    ...
    )
{
	...
	{
		SendNAF(extension,extension->GPIBAddress,
									  0,
									  0);
		ReadCSR(extension);
		ReadENCL(extension);
		TheChar = (UCHAR)ReadD(extension);
		extension->ReadString[extension->ActualStringPtr][extension->ActualStringPosPtr] = TheChar; 
		ReadCSR(extension);
		if((TheChar==(UCHAR)10)||(extension->ActualStringPosPtr > STRINGLEN))
		{
			extension->GPIBListening = FALSE;
			extension->GPIBPending   = FALSE;
			extension->ActualStringPosPtr = 0;
				...
				(extension->ActualInstructionPtr < INSTRUCTIONMEM))
		{
			...
			switch (InstructionType)
			{
				...
				case READGPIB:
					...
					extension->InstructionList[Index].type = 
						extension->InstructionList[Index].type | WAITINT;
					SendNAF(extension,extension->InstructionList[Index].n,
									0,
									24);
					ReadCSR(extension);
			...
			if ((extension->InstructionList[Index].type & WAITINT)!=0) break;
		} 
        ...
        case IRP_MJ_DEVICE_CONTROL:
			...
			case IOCTL_LISTDNAF:
				...
				if (extension->WriteInstructionPtr < INSTRUCTIONMEM)
				{
					...
					Status = STATUS_SUCCESS;
					pIOBuffer     = (PULONG)pIrp->AssociatedIrp.SystemBuffer;
					*(PUSHORT)pIOBuffer = extension->WriteInstructionPtr;
					pIrp->IoStatus.Information = sizeof(ULONG);
				}
			break;
			...
			case IOCTL_GPIBREADSTRING:
				Status = STATUS_UNSUCCESSFUL;
				if ((extension->ReadStringPtr < STRINGMEM)&&(extension->ReadStringPtr < extension->ActualStringPtr))
				{
					*(PUCHAR)pIOBuffer = extension->ReadString[extension->ReadDataPtr][0];
					extension->ReadDataPtr++;
					pIrp->IoStatus.Information = STRINGLEN;
					Status = STATUS_SUCCESS;
				}
			break;
